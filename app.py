...
def status():
    """
    Check if the vector database is loaded and ready.
    
    Returns:
        dict: Status of the database
    """
    if vector_database is None:
        return jsonify({
            'ready': False,
            'message': 'Vector database is being updated or is not available'
        })
    
    return jsonify({
        'ready': True,
        'message': 'Vector database is ready',
        'embedding_count': len(vector_database['embeddings'])
    })
